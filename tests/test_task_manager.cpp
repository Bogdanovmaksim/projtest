...
TEST_CASE("participant 2 addTask assigns ids and validates fields") {
    ...
    const Task first = manager.addTask("Доклад", parseDate("2026-06-10"), "учеба", Importance::High);
    const Task second = manager.addTask("Магазин", parseDate("2026-06-11"), "дом", Importance::Low);
    CHECK_EQ(first.id, 1);
    CHECK_EQ(second.id, 2);
    CHECK_EQ(manager.tasks().size(), 2U);
    CHECK_THROWS_AS(manager.addTask("", parseDate("2026-06-10"), "учеба", Importance::High),
...
TEST_CASE("participant 2 updateTask changes selected fields") {
    ...
    removeFiles(storage, archive);
    TaskManager manager(storage, archive);
    const Task created = manager.addTask("Старый текст", parseDate("2026-06-10"), "учеба",
                                         Importance::Low);
    TaskUpdate update;
    update.description = "Новый текст";
    update.deadline = parseDate("2026-06-15");
    ...
    CHECK_EQ(dateToString(changed.deadline), "2026-06-15");
...
TEST_CASE("participant 2 markDone and removeTask handle ids") {
    ...
    removeFiles(storage, archive);
    TaskManager manager(storage, archive);
    const Task first = manager.addTask("Первая", parseDate("2026-06-10"), "учеба", Importance::Low);
    const Task second =
        manager.addTask("Вторая", parseDate("2026-06-11"), "учеба", Importance::Medium);
...
TEST_CASE("participant 2 filterTasks and sortedByDeadline return correct lists") {
    ...
    removeFiles(storage, archive);
    TaskManager manager(storage, archive);
    manager.addTask("Поздняя домашка", parseDate("2026-06-20"), "учеба", Importance::Low);
    manager.addTask("Срочный проект", parseDate("2026-06-05"), "работа", Importance::High);
    manager.addTask("Средний проект", parseDate("2026-06-10"), "работа", Importance::Medium);
    TaskFilter filter;
...
TEST_CASE("participant 2 save and load persist tasks") {
    ...
    {
        TaskManager manager(storage, archive);
        manager.addTask("Первая", parseDate("2026-06-10"), "учеба", Importance::Low);
        manager.addTask("Вторая", parseDate("2026-06-11"), "работа", Importance::High);
        manager.save();
    }
    TaskManager loaded(storage, archive);
    loaded.load();
    CHECK_EQ(loaded.tasks().size(), 2U);
    CHECK_EQ(loaded.addTask("Третья", parseDate("2026-06-12"), "дом", Importance::Medium).id, 3);
    removeFiles(storage, archive);
}
...
TEST_CASE("participant 2 archiveCompleted moves only completed tasks") {
    ...
    removeFiles(storage, archive);
    TaskManager manager(storage, archive);
    const Task active = manager.addTask("Активная", parseDate("2026-06-10"), "учеба", Importance::Low);
    const Task done = manager.addTask("Готовая", parseDate("2026-06-11"), "работа", Importance::High);
